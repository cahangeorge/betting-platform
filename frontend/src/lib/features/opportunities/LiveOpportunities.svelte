<script lang="ts">
	...
	async function syncQueryToUrl(): Promise<boolean> {
		if (!browser || page.url.pathname !== '/opportunities' || page.url.searchParams.get('view') !== 'live') {
			return false;
		}
		...
		const nextUrl = `/opportunities?${query}`;
	...
	$effect(() => {
		...
		if (!browser || !didInitialize || pathname !== '/opportunities' || page.url.searchParams.get('view') !== 'live') {
			return;
		}
		...
		const timeout = setTimeout(() => {
			void (async () => {
				try {
					...
					if (!navigated && page.url.pathname === '/opportunities' && page.url.searchParams.get('view') === 'live') {
