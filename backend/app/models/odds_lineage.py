...
class OddsSnapshot(Base):
    ...
    scrape_job_id: Mapped[int | None] = mapped_column(
        ForeignKey("scrape_jobs.id", ondelete="SET NULL"), nullable=True, index=True
    )
...
class TicketLegQuoteSnapshot(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    ticket_leg_id: Mapped[int] = mapped_column(
        ForeignKey("ticket_legs.id", ondelete="CASCADE"), nullable=False, index=True
    )
